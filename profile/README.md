...
- [Workflows](#workflows)
    * [VCF Filtering and Rare Variant Burden Testing](#vcf-filtering-and-rare-variant-burden-testing)
        + [Provided Output Resources](#provided-output-resources)
            - [File Coordinate Reference](#file-coordinate-reference)
...
1. Filtered and annotated bcf files – generated following running "mrcepid-annotatecadd"
   * Stored in the folder `filtered_vcfs/` in `project-G6BJF50JJv8p4PjGB9yy7YQ2`.
2. Collapsed Variant Masks for Burden Testing – generated following "mrcepid-mergecollapsevariants"
   * Stored in the folder `collapsed_variants/` in `project-G6BJF50JJv8p4PjGB9yy7YQ2`.
...
This workflow also generates several additional resources, of which some are stored described here. These resources are
contained in the folder `project_resources/wes_resources/` in `project-G6BJF50JJv8p4PjGB9yy7YQ2`.

#### File Coordinate Reference

As one of the primary outputs of this pipeline, we provide a list of all filtered bcf chunks in the file: 

`coordinates.files.tsv.gz (file-G7YGYPjJJv8kz6QvP41q5KYg)` in `project-G6BJF50JJv8p4PjGB9yy7YQ2`.
...
This file is tab-delimited and tabix-indexed (.tbi) to enable searching for desired file-chunk(s).
...
`450k_vep.sorted.tsv.gz (file-G71V580JJv8f0x6KKv9gvGg4)` in `project-G6BJF50JJv8p4PjGB9yy7YQ2`.

This file is bgzipped and tabix-indexed (`file-G7j0kKjJJv8zJ00F0G93BGpg`) for easy querying with [tabix](http://www.htslib.org/doc/tabix.html). For the 
